...
def load_data(df_name: str) -> pd.DataFrame:
    ...
    if df_name == 'heart_failure_clinical_records_dataset':
        return load_heart_failure_clinical_records_dataset()

    if df_name == 'stroke_data':
        return load_stroke_data()

    raise ValueError(f'Unknown dataset name: {df_name}')
    


def load_heart_failure_clinical_records_dataset() -> pd.DataFrame:
    """Load heart_failure_clinical_records_dataset dataset from UCI repository"""
    column_names = [
        'age', 'anaemia', 'creatinine_phosphokinase',
        'diabetes', 'ejection_fraction', 'high_blood-pressure',
        'platelets', 'serum_cretinine', 'serum_sodium',
        'sex', 'smoking', 'time', 'death_event'
    ]
    #url = 'https://archive.ics.uci.edu/ml/machine-learning-databases/tic-tac-toe/tic-tac-toe.data'
    df = pd.read_csv("heart_failure_clinical_records_dataset.csv",names=column_names)
    df['death_event'] = [x == '1' for x in df['death_event']]
    return df
def load_stroke_data() -> pd.DataFrame:
    ...
    df['stroke'] = [x == '1' for x in df['stroke']]
    return df
